...
dat_all = read.csv("../../data/sa_species_data.csv")
...
lm_out = optim(c(.3, rnorm(p),rnorm(ns)) ,function(par){
  sum((log(y)  - par[1] - X_GDM %*% exp(par[2:(p + 1)])  )^2)
},method = "BFGS")
# 
# saveRDS(lm_out,"init_par_chi.rds")
# lm_out = readRDS("init_par_chi.rds")
...
# sig2_eta_save = numeric(reps) ; sig2_eta_now = readRDS("sig2_eta_now_chi.rds")
sig2_V = numeric(reps) ; sig2_V_now = 1
log_v_now = numeric(N)
# eta_save = matrix(NA,reps, ns) ; eta_now = readRDS("eta_now_chi.rds")
log_beta_save = matrix(NA,reps,p); log_beta_now = c(lm_out$par[2:(p+1)])
alpha_save = numeric(reps); alpha_now = lm_out$par[1]
# y_pred_save = matrix(0,reps,n_miss); y_pred_now = y_hold
...
xb = c(alpha_now + X_GDM %*% exp(log_beta_now)) #+ (eta_now[row_ind]  - eta_now[col_ind])^2)
# var(log(dat$y) - xb) /var(log(dat$y))

log_v_now[Y_is_not_one] = log(y[Y_is_not_one])
log_v_now[Y_is_one] = rtnorm(n1,mu = xb[Y_is_one],sd = sqrt(sig2_V_now),lb=0,ub= Inf)
# log_v_now[Y_is_missing] = rnorm(n_miss,mean = xb[Y_is_missing],sd = sqrt(sig2_V_now))
...
for(i in 1:(reps*thin + burn)){
  ...
  #               (eta_cand[row_ind]  - eta_cand[col_ind])^2)
  # 
  ...
  a_V = 1 + N/2
  b_V = 1 + sum((log_v_now - xb)^2)/2
  sig2_V_now = 1/rgamma(1,a_V,b_V)
...
#   pred_mean = apply(y_pred_save,2,mean)
#   
#   crps_out = mean(crps_sample(y_hold,t(y_pred_save)))
#   MAE_out = mean(abs(y_hold - pred_mean))
#   RMSE_out = sqrt(mean((y_hold - pred_mean)^2))
#   out = c(CRPS = crps_out, RMSE = RMSE_out, MAE = MAE_out)
#   return(out)
#   
# },mc.cores = 10)
elapsed = proc.time() - st

saveRDS(data.frame(model = 1,time_mins = elapsed[1]/60/24,like = log_lik),"mod1_sa.rds")

...
for(i in 1:(reps*thin + burn)){
  ...
  for(j in 1:(p+1)){
    ...
    X_sigma_cand = cbind(1,xb_cand,xb_cand^2,xb_cand^3)
    
    sig2_V_cand = exp(c(X_sigma_cand %*% log_beta_sigma_now))
    like_cand = sum(dnorm(log_v_now,xb_cand,sqrt(sig2_V_cand),log = TRUE))
